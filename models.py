...
class BaselineNet(nn.Module):
    ...
    def forward(self, image, question):
        ...
        img_embed = self.compute_vis_feats(image)
        img_embed = img_embed.view(img_embed.shape[0], -1)
        q_embed = self.compute_text_feats(question)
        q_embed = q_embed.view(q_embed.shape[0], -1)
        input_feats = torch.cat((img_embed, q_embed), dim=-1)
        output = self.classifier(input_feats)
        return output
    ...
    def compute_vis_feats(self, image):
        ...
        res_feats = self.vis_encoder(image)
        avg_pool = nn.AdaptiveAvgPool2d(1)
        vis_feats = avg_pool(res_feats)
        return vis_feats  # feed to vis_encoder and then mean pool on spatial dims
...
class BaselineNetDeep(nn.Module):
    """Simple baseline for VQA."""
...
class TransformerNet(BaselineNet):
    ...
    def __init__(self, n_answers=5217):
        ...
        # Cross-attention
        self.num_crossatt_layers = 3
        self.ca = nn.ModuleList([CrossAttentionLayer() for _ in range(self.num_crossatt_layers)])
...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model=256, dropout=0.1, n_heads=8):
        ...
        # FFN for seq1
        self.ffn_12 = nn.Sequential(nn.Linear(d_model,1024),
                      nn.ReLU(),
                      nn.Dropout(dropout),
        ...
        # FFN for seq2
        self.ffn_21 = nn.Sequential(nn.Linear(d_model,1024),
                      nn.ReLU(),
                      nn.Dropout(dropout),
                      nn.Linear(1024,d_model),
                      nn.Dropout(dropout),
    ...
    def forward(self, seq1, seq1_key_padding_mask, seq2,
        ...
        seq1_cros12 = self.norm_12(seq1 + self.dropout_12(cros12))
        seq1_ffn12 = self.ffn_12(seq1_cros12)
        seq1 = self.norm_122(seq1_cros12 + seq1_ffn12)
        
        # Cross-attention from seq2 to seq1 and FFN
        cros21 = self.cross_21(
            query=q2,
            key=k1,
            value=v1,
            attn_mask=None,
        ...
        seq2_cros21 = self.norm_21(seq2 + self.dropout_21(cros21))
        seq2_ffn21 = self.ffn_21(seq2_cros21)
        seq2 = self.norm_212(seq2_cros21 + seq2_ffn21)
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, spat_dim):
        ...
        dim_t = self.temperature ** (2 * torch.divide(dim_t, 2, rounding_mode = 'trunc') / self.num_pos_feats)
